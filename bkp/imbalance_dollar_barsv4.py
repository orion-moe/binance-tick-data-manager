...
def process_partition_imbalance_numba(
    ...
    time_reset_seconds = 3600000.0 * 1_000_000 * time_reset
    ...
    for i in range(len(prices)):
        ...
        if trigger_var >= threshold:
            ...
            if time_since_bar_start > time_reset_seconds:
                warm = True
                threshold = init_ticks
            else:
                threshold = ewma_T * ewma_imbalance
...
def process_files_and_generate_bars(
    ...
    alpha_imbalance=0.9, output_dir=None, time_reset
):
    ...
    output_file_prefix = f'{timestamp}-imbalance_dollar_bars_ticks{init_ticks}-aticks{alpha_ticks}-aimb{alpha_imbalance}-t{time_reset}'
...
def main():
    ...
    parser.add_argument('--time_reset', type=float, default=5, help='Tempo para resetar os params iniciais')
