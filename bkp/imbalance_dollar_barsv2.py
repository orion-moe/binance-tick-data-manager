...
def process_partition_imbalance_numba(
    ...
    for i in range(len(prices)):
        ...
        if abs(current_imbalance) >= threshold:
            ...
            P_tick_buy = ticks_buy / ticks

            P_tick_sell = (ticks_buy - ticks) / ticks

            expected_imbalance = abs(P_tick_buy * buy_volume_usd + P_tick_sell * (total_volume_usd - buy_volume_usd))
            ewma_imbalance += alpha_imbalance * (expected_imbalance - ewma_imbalance)
...
def process_files_and_generate_bars(
    ...
    output_file_prefix = f'imbalance_dollar_bars_ticks{init_ticks}-imb{init_imbalance}-aticks{alpha_volume}-aimb{alpha_imbalance}'
    ...
    system_state = (
        ...
        float(init_ticks), float(init_imbalance)
    )
    ...
    if all_bars:
        ...
        df_all_bars['end_time'] = pd.to_datetime(df_all_bars['end_time'], unit='us')
...
def main():
    ...
    parser.add_argument('--init_ticks', type=float, default=1_000_000, help='Ticks inicial esperado por barra')
    parser.add_argument('--init_imbalance', type=float, default=500, help='Imbalance inicial esperado por barra')
    parser.add_argument('--alpha_volume', type=float, default=0.9, help='Fator de decay para EWMA do volume')
    ...
    process_files_and_generate_bars(
        ...
        init_volume=args.init_ticks, init_imbalance=args.init_imbalance,
        alpha_volume=args.alpha_volume, alpha_imbalance=args.alpha_imbalance,
